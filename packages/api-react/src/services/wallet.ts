...
import { CAT, DID, Farmer, NFT, Pool, WalletService, WalletType, toBech32m, VC } from '@gold-network/api';
import type { NFTInfo, Transaction, Wallet, WalletBalance } from '@gold-network/api';
        ...
        try {
          ...
          return {
            ...
            data: await new Promise(async (resolve, reject) => {
              ...
              // make transaction
              const { data: stakeFromData, error } = await fetchWithBQ({
                command: 'stakeFrom',
                service: WalletService,
                args: restArgs,
              });
              ...
              if (!waitForConfirmation) {
                resolve(stakeFromData);
                return;
              }

              const { transaction } = stakeFromData;
              if (!transaction) {
                reject(new Error('Transaction is not present in response'));
                return;
              }
