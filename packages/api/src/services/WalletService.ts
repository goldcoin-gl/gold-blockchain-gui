...
export default class Wallet extends Service {
  ...
  async stakeInfo(args: {source: string}) {
    return this.command<{balance: number; address: string;}>('stake_info', args);
  }

  async stakeFrom(args: {
    walletId: number;
    amount: BigNumber;
    fee: BigNumber;
    address: string;
  }) {
    return this.command<{ transaction: Transaction; transactionId: string }>('stake_from', args);
  }
}
