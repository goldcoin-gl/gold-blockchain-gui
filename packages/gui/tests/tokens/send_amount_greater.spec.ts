...
test('Confirm that User cannot send a TGL amount greater then in Wallet', async () => {
  let receiveWallet = 'tgl1ksr59en3j3t9zaprmya0jp9k6mkazq2u5lfccaf85sej7kgmvjjscanfxd';
  ...
  console.log(`GL Balance: ${balance}`);
  ...
  //await page.locator('text=Fee *TGL >> input[type="text"]').fill('0.000005');
  ...
  await expect(page.getByText('Pending Balance0 TGL')).toBeVisible();
});
