import { Button, Flex, Loading, useOpenDialog, goldToMojo } from '@gold-network/core';
...
export default function WalletConnectCreateOfferPreview(props: WalletConnectOfferPreviewProps) {
  ...
  async function handleShowPreview() {
    ...
    if (offerBuilderDataResult) {
      // use new fee value
      const feeGold = offerBuilderDataResult.offered.fee?.[0]?.amount;
      if (feeGold) {
        const feeMojos = feeGold ? goldToMojo(feeGold).toFixed() : '0';
        onChange({
          ...values,
          fee: feeMojos,
        });
