// import { useGetNFTInfoQuery } from '@gold-network/api-react';
import { fromBech32m } from '@gold-network/api';
import { AddressBookContext, Color, EmojiAndColorPicker, Flex, Form, TextField } from '@gold-network/core';
...
export default function ContactEdit() {
  ...
  function handleSubmit(data) {
    ...
    filteredAddresses.forEach((entry) => {
      try {
        if (entry.address[2] === '1') {
          if (entry.address.slice(0, 2).toLowerCase() !== 'gl') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
        } else if (entry.address[3] === '1') {
          if (entry.address.slice(0, 3).toLowerCase() !== 'tgl') {
            throw new Error();
          } else if (fromBech32m(entry.address).length !== 64) {
            throw new Error();
          }
    ...
    filteredProfiles.forEach((entry) => {
      try {
        if (entry.did.slice(0, 9).toLowerCase() !== 'did:gold:') {
          throw new Error();
