import { useGetFarmedAmountQuery } from '@gold-network/api-react';
import {
  useCurrencyCode,
...
function FarmingRewardsHistoryCards() {
  ...
  const totalGoldFarmedCard = useMemo(() => {
    if (!data || isLoading) {
      return <CardSimple title={<Trans>Total GL Farmed</Trans>} value="-" loading={isLoading} error={error} />;
    }
    ...
    if (data && data.farmedAmount) {
        ...
        <StyledTable>
          <tbody>
            <tr>
              ...
              <td>
                {mojoToGoldLocaleString(data.poolRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToGoldLocaleString(data.farmerRewardAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
            <tr>
              ...
              <td>
                {mojoToGoldLocaleString(data.feeAmount, locale)}
                &nbsp;
                {currencyCode}
              </td>
            </tr>
      ...
      cardValue = (
        <Tooltip title={title}>
          {mojoToGoldLocaleString(data.farmedAmount, locale)}
          &nbsp;
          {currencyCode}
        </Tooltip>
      );
    ...
    return <CardSimple title={<Trans>Total GL Farmed</Trans>} value={cardValue} loading={isLoading} error={error} />;
