import { useGetBlockchainStateQuery, useGetTotalHarvestersSummaryQuery } from '@gold-network/api-react';
import { State, CardSimple, useCurrencyCode, mojoToGoldLocaleString, useLocale } from '@gold-network/core';
...
const MOJO_PER_GOLD = 1_000_000_000_000;
const BLOCKS_PER_YEAR = 1_681_920; // 32 * 6 * 24 * 365
function getBlockRewardByHeight(height: number) {
  if (height === 0) {
    return 21_000_000 * MOJO_PER_GOLD;
  }
  if (height < 3 * BLOCKS_PER_YEAR) {
    return 2 * MOJO_PER_GOLD;
  }
  if (height < 6 * BLOCKS_PER_YEAR) {
    return 1 * MOJO_PER_GOLD;
  }
  if (height < 9 * BLOCKS_PER_YEAR) {
    return 0.5 * MOJO_PER_GOLD;
  }
  if (height < 12 * BLOCKS_PER_YEAR) {
    return 0.25 * MOJO_PER_GOLD;
  }

  return 0.125 * MOJO_PER_GOLD;
}
...
function FarmingRewardsCards() {
  ...
  const estimatedDailyGLCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated daily GL</Trans>} state={state} />;
    }

    const estimatedDailyGL = new BigNumber(86_400)
    ...
    return (
      <CardSimple
        title={<Trans>Estimated daily GL</Trans>}
        value={
          <>
            {mojoToGoldLocaleString(estimatedDailyGL, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  const estimatedMonthlyGLCard = React.useMemo(() => {
    if (fullNodeState !== FullNodeState.SYNCED || !expectedTimeToWinSeconds || !data) {
      const state = fullNodeState === FullNodeState.SYNCHING ? State.WARNING : undefined;

      return <FarmCardNotAvailable title={<Trans>Estimated monthly GL</Trans>} state={state} />;
    }

    const estimatedMonthlyGL = new BigNumber(86_400 * 31)
      .div(expectedTimeToWinSeconds)
      .multipliedBy(getBlockRewardByHeight(data.peak.height))
      .dp(0);
    ...
    return (
      <CardSimple
        title={<Trans>Estimated monthly GL</Trans>}
        value={
          <>
            {mojoToGoldLocaleString(estimatedMonthlyGL, locale)}
            &nbsp;
            {currencyCode}
          </>
        }
  ...
  return (
    <Box>
      ...
      <Grid spacing={2} alignItems="stretch" container>
        ...
        <Grid xs={12} sm={6} md={4} item>
          {estimatedDailyGLCard}
        </Grid>
        <Grid xs={12} sm={6} md={4} item>
          {estimatedMonthlyGLCard}
        </Grid>
      </Grid>
    </Box>
  );
