...
type CreateDefaultValuesParams = {
  walletType?: WalletType; // CAT or STANDARD_WALLET (GL), indicates whether a token or CAT has a default entry
 ...
 * for creating an GL offer, a CAT offer, or an NFT offer. GL and CAT offers will have a default entry in
...
export default function createDefaultValues(params: CreateDefaultValuesParams | undefined = {}): OfferBuilderData {
  ...
  return {
    ...
    offered: {
      ...
      nfts,
      gl: walletType === WalletType.STANDARD_WALLET ? [{ amount: '' }] : [],
