import { ConfirmDialog, Flex } from '@gold-network/core';
...
function getSpendableAmountUponUnlockingAssets(assetStatus: AssetStatusForOffer, assetsToBeUnlocked: PendingAsset[]) {
  ...
  for (let s = 0; s < assetsToBeUnlocked.length; s++) {
    ...
    if (lockedAsset.type === 'GL' && assetStatus.type === 'GL') {
      // assetId might be either 'GL' or 'UNKNOWN'(maybe fee) but here both are treated as just a GL spending
