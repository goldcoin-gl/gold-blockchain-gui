import { Color, Flex } from '@gold-network/core';
import { Offering, Requesting } from '@gold-network/icons';
...
import OfferBuilderGLSection from './OfferBuilderGLSection';
...
export default function OfferBuilderTradeColumn(props: OfferBuilderTradeColumnProps) {
  ...
  const showGL = !readOnly || !!gl.length;
  ...
  const mutedGL = nfts.length || tokens.length;
  const mutedTokens = gl.length || nfts.length;
  const mutedNFTs = gl.length || tokens.length;
  ...
  return (
    <Flex flexDirection="column" gap={3}>
      ...
      >
        {showGL && <OfferBuilderGLSection name={`${name}.gl`} offering={offering} muted={mutedGL} />}
