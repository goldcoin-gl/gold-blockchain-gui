import { fungibleAssetFromAssetIdAndAmount, royaltyAssetFromNFTInfo } from '@gold-network/api';
import type { CalculateRoyaltiesRequest, NFTInfo } from '@gold-network/api';
import {
  useCalculateRoyaltiesForNFTsQuery,
  useGetNFTsByNFTIDsQuery,
  useGetWalletsQuery,
} from '@gold-network/api-react';
import { catToMojo, goldToMojo } from '@gold-network/core';
...
export default function OfferBuilderProvider(props: OfferBuilderProviderProps) {
  ...
  const requestedGL = useWatch({
    name: 'requested.gl',
  });

  const offeredGL = useWatch({
    name: 'offered.gl',
  });
  ...
  const requestedFungibleAssets = [
    ...(requestedGL ?? [])
      ...
      .map(({ amount }) => fungibleAssetFromAssetIdAndAmount('gl', goldToMojo(amount))),
    ...(requestedTokens ?? [])
