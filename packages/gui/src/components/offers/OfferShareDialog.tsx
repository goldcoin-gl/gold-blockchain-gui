import { OfferTradeRecord } from '@gold-network/api';
import {
  ButtonLoading,
  CopyToClipboard,
  DialogActions,
...
const log = debug('gold-gui:offers');
...
const testnetDummyHost = 'offers-api-sim.glcoin.net';
...
async function postToSpacescan(offerData: string, testnet: boolean): Promise<{ viewLink: string; offerLink: string }> {
  ...
  const requestOptions = {
    ...
    port: 443,
    path: `/api/offer/upload?coin=${testnet ? 'tgl' : 'gl'}&version=1`,
  };
  const requestHeaders = {
    'Content-Type': 'application/json',
  };
