import { OfferSummaryRecord, OfferTradeRecord } from '@gold-network/api';
import { useCheckOfferValidityMutation, useGetWalletsQuery } from '@gold-network/api-react';
import {
  Back,
  Button,
  ButtonLoading,
...
function NFTOfferSummaryRow(props: NFTOfferSummaryRowProps) {
  ...
  const assetIdsToTypes: { [key: string]: OfferAsset | undefined }[] = useMemo(
    () =>
      Object.keys(summaryData).map((key) => {
        ...
        if (['gl', 'tgl'].includes(key.toLowerCase())) {
  ...
  const rows: (React.ReactElement | null)[] = assetIdsToTypes.map((entry) => {
    ...
    switch (assetType) {
      case undefined:
        return null;
      case OfferAsset.GOLD: // fall-through
...
function NFTOfferDetails(props: NFTOfferDetailsProps) {
  ...
  const nftSaleInfo = useMemo(() => {
    ...
    const glMakerFee = mojoToGold(makerFee);

    return {
      ...calculateNFTRoyalties(
        amount,
        parseFloat(glMakerFee),
  ...
  const overrideNFTSellerAmount =
    exchangeType === NFTOfferExchangeType.TokenForNFT
      ? assetType === OfferAsset.GOLD
        ? goldToMojo(nftSaleInfo?.nftSellerNetAmount ?? 0)
