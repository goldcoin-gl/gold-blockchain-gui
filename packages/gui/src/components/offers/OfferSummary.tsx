import { type OfferSummaryRecord } from '@gold-network/api';
import { Flex, FormatLargeNumber, StateColor, TooltipIcon, mojoToGold, mojoToCAT } from '@gold-network/core';
...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount =
    makerEntries.length > 0
      ? ['gl', 'tgl'].includes(makerEntries[0][0].toLowerCase())
        ? mojoToGold(makerEntries[0][1])
        : mojoToCAT(makerEntries[0][1])
      : undefined;
  const takerAmount =
    takerEntries.length > 0
      ? ['gl', 'tgl'].includes(takerEntries[0][0].toLowerCase())
        ? mojoToGold(takerEntries[0][1])
