...
import type { CATToken, Wallet } from '@gold-network/api';
import { useGetCatListQuery, useGetWalletsQuery } from '@gold-network/api-react';
...
export default function NFTOfferTokenSelector(props: Props) {
  ...
  const [, options]: [TokenSelectOption, TokenSelectOption[]] = useMemo(() => {
    ...
    const goldWalletSelection = [wallets.find((wallet: Wallet) => wallet.type === WalletType.STANDARD_WALLET)].map(
      (wallet: WalletType) => ({
        walletId: wallet.id,
        walletType: wallet.type,
        name: 'Gold',
        symbol: currencyCode,
        displayName: `Gold (${currencyCode})`,
        disabled: false,
        tail: '',
      }),
    );
    ...
    const allOptions = [...goldWalletSelection, ...catOptions];
