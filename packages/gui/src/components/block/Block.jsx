...
import { useGetBlockQuery, useGetBlockRecordQuery } from '@gold-network/api-react';
import {
  Back,
  Button,
  Card,
...
export default function Block() {
  ...
  const poolReward = mojoToGold(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojoToGold(calculateBaseFarmerReward(blockRecord.height));

  const goldFees = blockRecord.fees !== undefined ? mojoToGold(blockRecord.fees) : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: goldFees ? `${goldFees} ${currencyCode}` : '',
  ...
  return (
    <LayoutDashboardSub>
      <Card
        title={
          <Back variant="h5">
            <Trans>Block at height {blockRecord.height} in the Gold blockchain</Trans>
          </Back>
        }
        action={
          <Flex gap={1}>
