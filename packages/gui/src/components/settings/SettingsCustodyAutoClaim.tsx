import { useGetAutoClaimQuery, useSetAutoClaimMutation } from '@gold-network/api-react';
import { Flex, SettingsText, Form, ButtonLoading, Fee, goldToMojo, mojoToGold } from '@gold-network/core';
import { ConnectCheckmark } from '@gold-network/icons';
...
export default function SettingsCustodyAutoClaim(props) {
  ...
  const autoClaimFee = autoClaimData?.txFee ? mojoToGold(autoClaimData.txFee).toNumber() : 0;
  ...
  async function handleSubmit({ fee }: { fee: number }) {
    const feeInMojos = goldToMojo(fee);
    await setAutoClaim({
      enabled: fee > 0,
      txFee: feeInMojos,
