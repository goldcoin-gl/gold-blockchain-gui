...
export default function ProfileAdd() {
  ...
  const currencyCode = (useCurrencyCode() ?? 'GL').toUpperCase();
  const isTestnet = currencyCode === 'TGL';
  ...
  const spendableBalance = mojoToGoldLocaleString(balance?.spendableBalance);
  const canCreateProfile = (balance?.spendableBalance ?? 0) > 0;

  function handleClick() {
    const url = `https://${isTestnet ? 'testnet11-faucet.glcoin.net' : 'faucet.glcoin.net'}/?address=${currentAddress}`;
    openExternal(url);
  }
              ...
              {!canCreateProfile && (
                <Flex flexDirection="column" gap={1}>
                  ...
                  <Link onClick={handleClick}>
                    <Trans>Get Mojos from the Gold Faucet</Trans>
                  </Link>
                </Flex>
              )}
              <Flex flexDirection="column" gap={1}>
