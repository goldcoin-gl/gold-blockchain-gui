...
export function getUserDataDir(): string {
  const goldRootPath = getConfigRootDir();
  const appName = app.getName();
  const userDataDir = path.join(goldRootPath, 'gui', appName);
  return userDataDir;
}

export function setUserDataDir(): void {
  const goldRootUserDataPath = getUserDataDir();

  migrateUserDataIfNecessary();

  console.info(`Setting user data directory to ${goldRootUserDataPath}`);
  app.setPath('userData', goldRootUserDataPath);
}
...
export function migrateUserDataIfNecessary() {
  ...
  const leveldbDestPath = path.join(goldRootUserDataPath, 'Local Storage', 'leveldb');
