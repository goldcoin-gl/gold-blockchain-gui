...
const log = initDebug('gold-gui:walletConnect');

const availableCommands = walletConnectCommands.map((command) => `gold_${command.command}`);
...
export async function processSessionProposal(
  ...
  try {
    ...
    const requiredNamespace = requiredNamespaces.gold;
    if (!requiredNamespace) {
      throw new Error('Missing required gold namespace');
    }

    const { chains, methods } = requiredNamespace;
    const chain = chains.find((item) => ['gold:testnet', 'gold:mainnet'].includes(item));
    if (!chain) {
    ...
    const accounts = fingerprints.map((fingerprint) => `gold:${instance}:${fingerprint}`);
