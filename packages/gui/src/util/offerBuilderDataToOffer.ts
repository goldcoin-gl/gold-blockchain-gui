...
import { goldToMojo, catToMojo } from '@gold-network/core';
...
export default async function offerBuilderDataToOffer({
  ...
  const {
    offered: { gl: offeredXch = [], tokens: offeredTokens = [], nfts: offeredNfts = [], fee: [firstFee] = [] },
    requested: { gl: requestedXch = [], tokens: requestedTokens = [], nfts: requestedNfts = [] },
  } = data;

  const usedNFTs: string[] = [];

  const feeInMojos = firstFee ? goldToMojo(firstFee.amount) : new BigNumber(0);
        ...
        if (assetId.toUpperCase() === 'GL' || assetId.toUpperCase() === 'UNKNOWN') {
          ...
          if (idx > -1) {
            ...
            if (pendingOffers[idx].assetId.toUpperCase() !== assetId.toUpperCase()) {
              // Now we can distinguish that we have gl spending which is only GL, only Fee or both GL and Fee
              pendingOffers[idx].assetId = 'GL+FEE';
            }
          } else {
            pendingOffers.push({
              type: 'GL',
  ...
  const glTasks = offeredXch.map(async (gl) => {
    const { amount } = gl;
    if (!amount || amount === '0') {
      throw new Error(t`Please enter an GL amount`);
    }
    if (!standardWallet || !standardWalletBalance) {
      throw new Error(t`No standard wallet found`);
    }

    const mojoAmount = goldToMojo(amount);
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Amount exceeds GL total balance`);
    }
  ...
  if (offeredXch.length === 0 && feeInMojos.gt(0)) {
    ...
    if (!hasEnoughTotalBalance) {
      throw new Error(t`Fee exceeds GL total balance`);
    }
    if (pendingXchOffer) {
  ...
  await Promise.all([...glTasks, ...tokenTasks, ...nftTasks]);

  // requested
  requestedXch.forEach((gl) => {
    const { amount } = gl;
    ...
    walletIdsAndAmounts[wallet.id] = goldToMojo(amount);
  });
