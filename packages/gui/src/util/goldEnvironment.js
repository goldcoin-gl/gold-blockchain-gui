...
const PY_DIST_EXECUTABLE = 'gold';
const PY_DIST_EXEC_ARGS = Object.freeze(['start', 'daemon', '--skip-keyring']);

const PY_DEV_EXECUTABLE = `../../../venv/${process.platform === 'win32' ? 'Scripts/gold.exe' : 'bin/gold'}`;
...
const startGoldDaemon = () => {
  ...
  if (!pyProc) {
    throw new Error('Failed to start gold daemon');
  }
