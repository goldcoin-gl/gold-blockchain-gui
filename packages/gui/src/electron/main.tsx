...
import { NFTInfo } from '@gold-network/api';
...
import AppIcon from '../assets/img/gold64x64.png';
import About from '../components/about/About';
import { i18n } from '../config/locales';
import goldEnvironment, { goldInit } from '../util/goldEnvironment';
...
// At that time, the config file is created here by `gold init`.
if (!checkConfigFileExists()) {
  goldInit();
}

// Set the userData directory to its location within GOLD_ROOT/gui
setUserDataDir();
...
const ensureCorrectEnvironment = () => {
  ...
  if (!goldEnvironment.guessPackaged() && !('VIRTUAL_ENV' in process.env)) {
    app.quit();
    return false;
  }
...
if (ensureSingleInstance() && ensureCorrectEnvironment()) {
  ...
  const createWindow = async () => {
    if (manageDaemonLifetime(NET)) {
      goldEnvironment.startGoldDaemon();
    }
...
function getMenuTemplate() {
  const template = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: i18n._(/* i18n */ { id: 'Gold Blockchain Wiki' }),
          click: () => {
            openExternal('https://github.com/goldcoin-gl/gold-blockchain/wiki');
          },
        },
        {
          label: i18n._(/* i18n */ { id: 'Frequently Asked Questions' }),
          click: () => {
            openExternal('https://github.com/goldcoin-gl/gold-blockchain/wiki/FAQ');
          },
        },
        {
          label: i18n._(/* i18n */ { id: 'Release Notes' }),
          click: () => {
            openExternal('https://github.com/goldcoin-gl/gold-blockchain/releases');
          },
        },
        {
          label: i18n._(/* i18n */ { id: 'Contribute on GitHub' }),
          click: () => {
            openExternal('https://github.com/goldcoin-gl/gold-blockchain/blob/main/CONTRIBUTING.md');
          },
        },
      ],
    },
