import { SyncingStatus, toBech32m, WalletType } from '@gold-network/api';
import { useSpendCATMutation, useFarmBlockMutation } from '@gold-network/api-react';
import {
  AdvancedOptions,
  Button,
  EstimatedFee,
...
export default function WalletCATSend(props: Props) {
  ...
  async function handleSubmit(data: SendTransactionData) {
    ...
    if (address.includes('colour')) {
      throw new Error(t`Cannot send gold to coloured address. Please enter a gold address.`);
    }

    if (address.includes('gold_addr') || address.includes('colour_desc')) {
    ...
    if (address.slice(0, 12) === 'gold_addr://') {
