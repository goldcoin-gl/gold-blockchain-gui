import { WalletType, TransactionType, TransactionTypeFilterMode, toBech32m } from '@gold-network/api';
...
import type { Row } from '@gold-network/core';
...
const getCols = (type: WalletType, isSyncing, getOfferRecord, navigate, location) => [
    ...
    field: (row: Row, metadata) => {
      ...
      return (
        <>
          ...
          <strong>
            <FormatLargeNumber
              value={[WalletType.CAT, WalletType.CRCAT].includes(type) ? mojoToCAT(row.amount) : mojoToGold(row.amount)}
            />
          </strong>
          &nbsp;
          {metadata.unit}
...

  {
    field: (row: Row, metadata) => (
      <>
        <strong>
          <FormatLargeNumber value={mojoToGold(row.feeAmount)} />
        </strong>
        &nbsp;
        {metadata.feeUnit}
      </>
