import { TransactionType, WalletType } from '@gold-network/api';
import type { Transaction } from '@gold-network/api';
import { useGetWalletBalanceQuery } from '@gold-network/api-react';
import { Color, mojoToGold, mojoToCAT, blockHeightToTimestamp } from '@gold-network/core';
  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(
        0,
        ([WalletType.CAT, WalletType.CRCAT].includes(walletType) ? mojoToCAT(start) : mojoToGold(start)).toNumber(),
  ...
  data.forEach((item) => {
    ...
    points.push({
      ...
      tooltip: [WalletType.CAT, WalletType.CRCAT].includes(walletType)
        ? mojoToCAT(start)
        : mojoToGold(start).toString(), // bignumber is not supported by react
    });
  });
