import { useLocalStorage } from '@gold-network/api-react';
...
export default function useGetLatestVersionFromWebsite(): UseGetLatestVersionFromWebsiteResult {
  ...
  const [latestVersionURL] = useLocalStorage<string>(
    'latestVersionURL',
    'https://download.glcoin.net/latest/latest.json',
  );
  ...
  const downloadUrl = downloadPath ? new URL(downloadPath, 'https://www.glcoin.net/').toString() : undefined;
  const releaseNotesUrl = releaseNotesPath ? new URL(releaseNotesPath, 'https://www.glcoin.net/').toString() : undefined;
  const blogUrl = blogPath ? new URL(blogPath, 'https://www.glcoin.net/').toString() : undefined;
