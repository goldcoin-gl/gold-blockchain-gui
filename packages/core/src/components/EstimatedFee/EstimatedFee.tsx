import { useGetFeeEstimateQuery } from '@gold-network/api-react';
...
import mojoToGoldLocaleString from '../../utils/mojoToGoldLocaleString';
import Fee from '../Fee';
...
export enum FeeTxType {
  walletSendGL = 'send_gl_transaction',
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const formattedEstimates: FormattedEstimate[] = useMemo(() => {
    ...
    return estList.map((estimate: number, i: number) => {
      const formattedEstimate = mojoToGoldLocaleString(estimate, locale);
  ...
  useEffect(() => {
    if (formattedEstimates && inputType === 'dropdown') {
      ...
      if (selectedTime) {
        ...
        if (estimate) {
          const glFee = mojoToGoldLocaleString(estimate.estimate, 'en-US');
